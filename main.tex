...
Fusion approaches aim to integrate multimodal information through strategies such as early concatenation, late decision fusion, cross-modal attention, and transformer-based fusion blocks~\cite{zadeh2017tfn,tsai2019mult}.

\begin{itemize}
  \item \textbf{Early / Late Fusion:} Simple combination of modality features at feature or decision level~\cite{zadeh2017tfn}.
  \item \textbf{Attention-Based Fusion:} Mechanisms that dynamically emphasize salient modality cues~\cite{tsai2019mult}.
  \item \textbf{Transformer-Based Fusion:} Joint self- and cross-attention layers for richer modality interaction~\cite{tsai2019mult}.
  \item \textbf{Emotion-Shift-Aware Fusion:} Explicitly models sentiment changes across turns~\cite{li2024graphsmile}.
\end{itemize}
...
A more recent line of work incorporates auxiliary objectives and enhanced graph learning. \textbf{Joyful} integrates modality fusion with contrastive learning within a graph framework, constructing multimodal interaction graphs and employing contrastive objectives to encourage discriminative representations across modalities, addressing traditional issues such as over-smoothing in graph embeddings and improving cross-modal alignment.

\textbf{GraphSmile}~\cite{li2024graphsmile} represents a state-of-the-art graph-centric MERC architecture introducing two key components: \textit{Graph Structure Fusion (GSF)}, which constructs bipartite dialogue graphs between every pair of modalities and alternates information propagation across these graphs to prevent any single structural path from dominating; and \textit{Sentiment Dynamics Perception (SDP)}, an auxiliary task that explicitly predicts whether sentiment shifts occur between consecutive utterances to guide the model toward emotional transitions.
